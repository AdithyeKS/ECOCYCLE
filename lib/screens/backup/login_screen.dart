...
class _LoginScreenState extends State<LoginScreen> {
  ...
  String? _error;
  final _profileService = ProfileService(); // NEW INSTANCE
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (user != null) {
          // --- CRITICAL FIX START ---
          // 1. Force RLS check and ensure profile exists/is up-to-date
          var profileData = await _profileService.fetchProfile(user.id);
          var fetchedRole = profileData?['role']?.toString().toLowerCase() ?? 'user';
          ...
          // 2. Based on the role fetched HERE, decide the immediate navigation route
          if (mounted) {
              if (fetchedRole == 'admin' || fetchedRole == 'agent') {
                  // Direct route for privileged users, bypassing HomeShell's delay
                  Navigator.pushReplacement(
                      context,
                      MaterialPageRoute(
                          // HomeShell handles the final determination and displays the right dashboard
                          builder: (_) => HomeShell(toggleTheme: widget.onThemeToggle ?? () {})
                      )
                  );
              } else {
                  // Standard user route

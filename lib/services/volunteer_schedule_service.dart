...
class VolunteerScheduleService {
  ...
  Future<List<VolunteerSchedule>> fetchVolunteerSchedules(String volunteerId) async {
    final response = await _supabase
        .from('volunteer_schedules')
        .select()
        .eq('volunteer_id', volunteerId);
    
    return (response as List).map((json) => VolunteerSchedule.fromJson(json)).toList();
  }

  /// Sets or updates a volunteer's availability for a specific date.
  Future<void> setAvailability(String volunteerId, DateTime date, bool isAvailable) async {
  ...
  Future<List<VolunteerAssignment>> fetchVolunteerAssignments(String volunteerId) async {
    final response = await _supabase
        .from('volunteer_assignments')
        .select()
        .eq('volunteer_id', volunteerId);
    
    return (response as List).map((json) => VolunteerAssignment.fromJson(json)).toList();
  }
  ...
  Future<List<VolunteerSchedule>> getAvailableVolunteersInDateRange(DateTime start, DateTime end) async {
  ...
  Future<List<String>> getAvailableVolunteerIdsForDates(List<DateTime> dates) async {
    ...
    return (response as List).map((json) => json['volunteer_id'] as String).toSet().toList();
  }
